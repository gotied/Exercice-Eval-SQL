...
(2)	SELECT cli_nom,cli_prenom,cli_ville FROM client WHERE cli_id=4	
...
(6) SELECT hot_nom, hot_categorie, hot_ville FROM hotel WHERE hot_ville = 'bretou' AND hot_categorie >3

(7) SELECT hot_nom, hot_categorie, hot_ville, sta_nom FROM hotel 
...
(9) SELECT hot_nom, hot_categorie, hot_ville, cha_numero, cha_capacite 
FROM hotel 
JOIN chambre ON hot_id = cha_hot_id
WHERE cha_capacite >1 AND hot_ville = 'bretou'

(10) SELECT res_date, cli_nom, hot_nom FROM reservation
...
(12) SELECT cli_nom, hot_nom, res_date_debut, DATEDIFF (res_date_debut, res_date_fin) FROM reservation
JOIN client on cli_id = res_cli_id
JOIN chambre ON cha_id = res_cha_id
JOIN hotel ON hot_id = cha_hot_id

(13) SELECT sta_nom, COUNT(hot_nom) FROM hotel 
JOIN station ON hot_sta_id = sta_id 
GROUP BY sta_nom

(14) SELECT sta_nom, COUNT(cha_id) FROM station
JOIN hotel ON sta_id = hot_sta_id
JOIN chambre ON hot_id = cha_hot_id
GROUP BY sta_nom

(15) SELECT sta_nom, COUNT(cha_id) FROM station
JOIN hotel ON sta_id = hot_sta_id
JOIN chambre ON hot_id = cha_hot_id
WHERE cha_capacite >1
GROUP BY sta_nom 

(16) SELECT hot_nom, cli_nom FROM reservation
JOIN client ON res_cli_id = cli_id 
JOIN chambre ON res_cha_id = cha_id
JOIN hotel ON cha_hot_id = hot_id 
WHERE cli_id=6

(17) SELECT sta_nom, avg(datediff(res_date_debut,res_date_fin)) FROM reservation
JOIN chambre ON cha_id = res_cha_id
JOIN hotel ON hot_id = cha_hot_id
